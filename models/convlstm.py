...
def ConvLSTM(offset_x):
    ...
    x = L.ConvLSTM2D(64, (3,3), strides=2, padding='same', activation='relu', kernel_initializer='he_uniform', return_sequences=True)(x)
    encoded = L.ConvLSTM2D(128, (3,3), strides=2, padding='same', activation='relu', kernel_initializer='he_uniform', return_sequences=False, name='encoded')(x)

    x = L.Conv2DTranspose(128, (2, 2), strides=2, padding='same', activation='relu', kernel_initializer='he_uniform')(encoded)
...
def Encoder(convlstm):
    encoder = Model(
        input = convlstm.get_layer('inputs').input,
        output = convlstm.get_layer('encoded').output)
    return encoder
